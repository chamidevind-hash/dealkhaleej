...
async function readJsonArray(filePath) {
    ...
    if (error.code === "ENOENT") {
      try {
        ...
        if (createError.code !== "EEXIST") {
          console.error(`Failed to create ${path.basename(filePath)}:`, createError);
        }
      }
      return [];
    }
  ...
  try {
    ...
    if (!Array.isArray(data)) {
      console.error(`Invalid data in ${path.basename(filePath)}: expected a JSON array.`);
      return [];
    }
    return data;
  } catch (error) {
...
async function readStores() {
  return readJsonArray(storesFile);
}

async function readClicks() {
  return readJsonArray(clicksFile);
}

async function readOutboundClicks() {
  return readJsonArray(outboundClicksFile);
}

async function readImportHistory() {
  return readJsonArray(importHistoryFile);
}

async function readNewsletter() {
  return readJsonArray(newsletterFile);
}

async function readArticles() {
  return readJsonArray(articlesFile);
}
